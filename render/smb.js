...
import { getSmbOverride } from '../utils/source-overrides.js';

export function renderSMB() {
  const d = gd('smb');
  const override = getSmbOverride(d.months, d);
  ...
  bar('ch-smb-hq', d.months, [
    { label: '강북', data: override.본부별_강북 },
    { label: '강남', data: override.본부별_강남, color: C.teal, bg: C.tealA },
    { label: '강서', data: override.본부별_강서, color: C.blue, bg: C.blueA },
    { label: '동부', data: override.본부별_동부, color: C.purple, bg: C.purpleA },
    { label: '서부', data: override.본부별_서부, color: C.orange, bg: C.orangeA },
  ], true);

  line('ch-smb-hr', d.months, [
    { label: '전담 인력', data: d.인력_총원, fill: true },
  ]);
}
